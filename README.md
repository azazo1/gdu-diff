...
其中, 读取快照时会直接显示实际读取进度。对于持久化的 `.json.zst` 文件, 读取阶段会走流式 zstd 解压, 加载界面会显示压缩文件已读取的字节数和百分比, 然后再进入 JSON 解析阶段。因为解压本身就是流式的, 不需要额外先整块解压到内存, 也不会额外插入一个独立的"解压进度条"步骤。当前对比流程里刚扫描出来的临时 `current.json` 不会再压缩。
