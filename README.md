...
`shot` 和“当前目录 vs 历史快照”的模式都会直接调用外部 `gdu-go`/`gdu` 先生成 JSON, 然后在进程内用 zstd 最高压缩级别写成 `.json.zst` 快照。
...
其中, 读取快照时会直接显示实际读取进度。对于 `.json.zst` 文件, 读取阶段会走流式 zstd 解压, 加载界面会显示压缩文件已读取的字节数和百分比, 然后再进入 JSON 解析阶段。因为解压本身就是流式的, 不需要额外先整块解压到内存, 也不会额外插入一个独立的"解压进度条"步骤。
...
如果底层调用的是 `gdu-go` / `gdu` 扫描当前目录, 加载界面会尽量展示其扫描状态。同时会给扫描步骤补一个非线性的假进度, 先增长, 在 `95%` 停住, 等外部扫描真正完成并压缩落盘后再补到 `100%`。实际能否看到更细粒度的实时状态, 仍然取决于外部 `gdu-go` / `gdu` 在当前终端环境中的输出行为。
...
默认保存的新快照文件后缀是 `.json.zst`。程序仍然兼容直接读取旧的 `.json` 快照。
