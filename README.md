...
其中, 读取较大的 JSON 快照时, 程序会按文件大小和一组偏保守的固定常数估算加载时长, 用这个估算值展示一个假进度条, 避免读取阶段长时间卡在 0%.
...
如果底层调用的是 `gdu-go` / `gdu` 扫描当前目录, 加载界面会尽量展示其扫描状态。实际能否看到更细粒度的实时进度, 取决于外部 `gdu-go` / `gdu` 在当前终端环境中的输出行为。
